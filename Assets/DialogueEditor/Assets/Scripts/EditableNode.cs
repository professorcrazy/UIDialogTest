...
namespace DialogueEditor
{
    ...
    public class EditableSpeechNode : EditableConversationNode
    {
        ...
        public override void SerializeAssetData(NPCConversation conversation)
        {
            ...
            conversation.GetNodeData(this.ID).video = this.video;
        ...
        public override void DeserializeAssetData(NPCConversation conversation)
        {
            ...
            this.video = conversation.GetNodeData(this.ID).video;
            ...
            if (conversation.Version < (int)eSaveVersion.V1_03)
            {
                if (this.video == null)
                {
                    ...
                    {
                        string path = UnityEditor.AssetDatabase.GUIDToAssetPath(VideoGUID);
                        this.video = (VideoClip)UnityEditor.AssetDatabase.LoadAssetAtPath(path, typeof(VideoClip));
